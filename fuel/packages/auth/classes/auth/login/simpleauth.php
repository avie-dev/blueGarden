...
//← avie code starts here→ ==================||||||||||||||||||||||||||||||\(*^0^*)/|||||||||||||||||||||||||||||||========================
	public function get_all($rec_no)
	{
		$current_values = \DB::select_array(\Config::get('simpleauth.table_columns', array('*')))
			->from(\Config::get('simpleauth.table_name'))
			->where('group', '=', '1')
			...
			foreach($current_values as $currentval){
				
				$try = @unserialize($currentval['profile_fields']) ?: array(); /*Serialize profile_fields put it in array*/
				
				$records = array();
				
				if ($try !== false) {/*if exist */
					$records[$i]= $try;
			 
					if ($rec_no==$i){
						\Session::set('authrec',$try);
					}	
				} else {
		...
		$current_values = \DB::select_array(\Config::get('simpleauth.table_columns', array('*')))
			...
			foreach($current_values as $currentval){
				...
				if ($try !== false) {/*if exist */
						...
						\Session::set('authrec_id',$try);
						\Session::set('authid',$currentval['id']);
						\Session::set('authemail',$currentval['email']);
						\Session::set('authmember_status',$currentval['member_status']);
						\Session::set('authregdate',$currentval['created_at']);
				} else {
					echo "Record not Exist.";
				}
	...
	public function update_memrec($id,$email,$name,$kana,$tel,$post_no,$pref,$addr1,$addr2,$sex,$birthy,$birthm,$birthd)
	{
		$update = array();
		$update['email'] = $email;
		
		\Arr::set($profile_fields, array("name" => $name,
										"kana" => $kana,
										"tel" => $tel,
										"post_no" => $post_no,
										"pref" =>$pref,
	...
	public function get_by_name($search_val)
	{
		$current_values = \DB::select_array(\Config::get('simpleauth.table_columns', array('*')))
			->from(\Config::get('simpleauth.table_name'))
			->where('del_flag', '=', 0)
