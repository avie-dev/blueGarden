...
<body class="MyGradientClass">
	<div id="bluegarden_container">
		...
		<!--	追加		-->
			...
			<table width="98%" id="datetable" border ="1"><!-- Calendar Table-->			
			<tr>
			<p>◎ の日時から施術を開始することが出来ます。ご希望の来店日時の ◎ を選択してください。</p>
				...
				<?php
					...
					//echo $tomorrow; //already contains tomorrows date
					$selected_month = date($tomorrow);
					$showyear = \Model\Calendar::display_year($selected_month);
					$showmonth = \Model\Calendar::display_month($selected_month);
					$showday = \Model\Calendar::display_day($selected_month); ?>
					<th style="padding-right:5px; border-color:blue;font-size:12px;"><center>
					<?php echo '前の週'; ?>
					</center></th>
					
					<?php
					foreach($showday as $showdays)
					{
						$ymd = $showyear.'-'.$showmonth.'-'.$showdays;
						if($ymd==$tomorrow){
							for($counter=1;$counter<8;$counter++){
							?>
							<th style="width:50px; border-color:blue;color:blue;text-align:center;"><h6><p><center>
								<?php
								$ymd = mktime(0,0,0,date("m"),date("d")+$counter,date("Y"));
								$datelist = date("m/d", $ymd);
								?><span style="padding-left:-5px"><?php echo $datelist."&nbsp;&nbsp;&nbsp;&nbsp;"."<br>"; ?></span>
								<?php $showweekday = \Model\Calendar::display_dayofweek($datelist);
								?><p>
								<?php echo "(".$showweekday.")"; ?>
								</p>
							</center></p></h6></th>
							<?php
							}
					...
					<?php echo Html::anchor('admin/weektwo', '次の週    '); ?>
					</center></th>
					<?php
				?>
				</tr>
				...
				if ($result) {
				  ...
				  if(floor($converted) != $converted){
				    ...
				    echo $converted; print ":00";
						...
						<?php
							if($counter==1){
								$unformatted=strtotime(date('Y-m-d', strtotime($datelist)) . ' -6 days');
